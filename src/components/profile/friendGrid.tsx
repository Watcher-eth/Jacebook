      ...
      <div className="grid grid-cols-1 gap-3">
        {friends.map((f) => {
          ...
          const appearancesLabel =
            f.weight >= 9000 || f.slug === EPSTEIN_SLUG ? null : `${f.weight} shared appearances`;
          ...
          return (
            <div key={f.slug} className="flex items-center gap-3 border border-border rounded-lg p-2 bg-white">
              <Avatar className="h-[72px] w-[72px] rounded-sm flex-shrink-0">
                <AvatarImage className="rounded-sm object-cover" src={src} />
              ...
              <div className="min-w-0 flex-1">
                ...
                {appearancesLabel ? (
                  <div className="text-xs text-muted-foreground mt-0.5">{appearancesLabel}</div>
                ) : (
                  <div className="text-xs text-muted-foreground mt-0.5">Friend</div>
                )}
              </div>
